...
tem_cols_all <- c("bla TEM_1", "blaTEM_1A", "blaTEM_1B")
...
# Statistical test
wilcox_tem <- wilcox.test(mer_tem_positive, mer_tem_negative)
fc_tem <- median(mer_tem_positive) / median(mer_tem_negative)
```
...
tem_summary <- data.frame(
  ...
  Median_MIC = c(median(mer_tem_positive), median(mer_tem_negative)),
  Mean_MIC = c(mean(mer_tem_positive), mean(mer_tem_negative)),
  Range = c(paste(range(mer_tem_positive), collapse = "–"),
            paste(range(mer_tem_negative), collapse = "–")),
  Percent_Susceptible = c(
    sprintf("%.1f%%", 100 * sum(mer_tem_positive <= 2) / length(mer_tem_positive)),
    sprintf("%.1f%%", 100 * sum(mer_tem_negative <= 2) / length(mer_tem_negative))
  ),
  Percent_Resistant = c(
    sprintf("%.1f%%", 100 * sum(mer_tem_positive > 2) / length(mer_tem_positive)),
    sprintf("%.1f%%", 100 * sum(mer_tem_negative > 2) / length(mer_tem_negative))
...
kable(tem_summary,
  ...
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) |>
  add_footnote(sprintf("Wilcoxon rank-sum test: W = %.1f, p = %.4f (trend)", 
                      wilcox_tem$statistic, wilcox_tem$p.value),
               notation = "none")
```
