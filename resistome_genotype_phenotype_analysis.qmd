...
analysis_data$n_betalactamases <- rowSums(gene_matrix[, -1], na.rm = TRUE)
...
gene_prev <- colSums(gene_matrix[, -1], na.rm = TRUE)
...
gene_classification <- c(
  "blaSHV-28" = "ESBL",
  "BlaCTXM-15" = "ESBL", 
  "blaOXA-1" = "Narrow-spectrum",
  "blaTEM-1B" = "Narrow-spectrum",
  "bla_TEM-1" = "Narrow-spectrum",
  "blaTEM-1A" = "Narrow-spectrum",
  "bla_OXA-9" = "Narrow-spectrum",
  "blaSHV-1" = "Narrow-spectrum",
  "blaSHV-11" = "ESBL",
  "bla_OXA-18" = "Narrow-spectrum",
  "BlaSHV-106" = "ESBL",
  "BlaSHV-158" = "ESBL",
...
gene_prevalence_summary <- data.frame(
  Gene = names(gene_prev_sorted),
  n = as.integer(gene_prev_sorted),
  Percentage = round(100 * gene_prev_sorted / nrow(gene_matrix), 1)
) |>
  filter(n > 0) |>
  mutate(Classification = gene_classification[Gene])
...
gene_comparison_plot <- gene_species_comparison |>
  ...
  mutate(
    ...
    Gene = factor(Gene, levels = unique(Gene[order(KP_pct + EC_pct, decreasing = TRUE)]))
  ) |>
  filter(Percentage > 0)
...
for(gene in major_genes) {
  key_antibiotics_analysis <- c("CAZ-AVI", "CAZ", "FEP", "ATM", "C/T", "PIP- TAZ", "MER")
...
plot_data <- analysis_data |>
  select(ID, `blaOXA-1`, bla_TEM-1, `CAZ-AVI`, ATM, MER, `PIP- TAZ`) |>
  pivot_longer(cols = c(`CAZ-AVI`, ATM, MER, `PIP- TAZ`),
               names_to = "Antibiotic",
               values_to = "MIC") |>
  pivot_longer(cols = c(`blaOXA-1`, bla_TEM-1),
               names_to = "Gene",
  ...
  mutate(
    ...
    Gene = case_when(
      Gene == "blaOXA-1" ~ "blaOXA-1",
      Gene == "bla_TEM-1" ~ "bla_TEM-1"
    )
  )
...
for(profile in names(top_profiles)) {
  ...
  caz_avi_median <- median(profile_data$`CAZ-AVI`, na.rm = TRUE)
  caz_avi_range <- paste0("[", 
                          round(min(profile_data$`CAZ-AVI`, na.rm = TRUE), 0), 
                          "-", 
                          round(max(profile_data$`CAZ-AVI`, na.rm = TRUE), 0), 
                          "]")
  
  profile_summary <- rbind(profile_summary, data.frame(
    Profile = ifelse(profile == "None", "None (KPC-31 only)", 
                    gsub("\\+", " + ", profile)),
...
1. **KPC-31-only isolates (45%):**
   - Median CAZ-AVI MIC = `{r} median(analysis_data$'CAZ-AVI'[analysis_data$n_betalactamases == 0], na.rm=TRUE)` mg/L
...
burden_mic_data <- analysis_data |>
  filter(!is.na(`CAZ-AVI`)) |>
  mutate(
    n_genes_factor = factor(n_betalactamases),
    log2_CAZ_AVI = log2(`CAZ-AVI` + 0.1)
  )
...
key_antibiotics_species <- c("CAZ-AVI", "CAZ", "FEP", "ATM", "C/T", "PIP- TAZ", "MER", "IMI")
...
kable(species_comparison,
      ...
      digits = 4) |>
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) |>
  column_spec(6, background = ifelse(species_comparison$p_value < 0.1, "#FFF3CD", "white"))
```
